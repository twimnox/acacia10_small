...
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 178

# Constants describing the training process.
MOVING_AVERAGE_DECAY = 0.9999     # era 0.9999The decay to use for the moving average.
NUM_EPOCHS_PER_DECAY = 150   #era 350   # Epochs after which learning rate decays.
...
def _activation_summary(x):
  ...
  tensor_name = re.sub('%s_[0-2]*/' % TOWER_NAME, '', x.op.name) # era [0-9]
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights', shape=[6, 6, 3, 64], #era 5, 5, 3, 64
                                         stddev=1e-4, wd=0.0)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    ...
    softmax_linear = tf.nn.xw_plus_b(local3, weights, biases, name=scope.name) #ERA LOCAL4
